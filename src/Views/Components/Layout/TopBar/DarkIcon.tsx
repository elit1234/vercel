...
const DarkIcon = () => {
  ...
  const initAnimRef = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    const initTarget = isDark ? ".sunIcon" : ".moonIcon";
    initAnimRef.current = initTl.fromTo(
      initTarget,
      {
        y: -50,
        autoAlpha: 0,
  ...
  const clickedOuter = () => {
    ...
    console.log(isDark);
    //sun is out, ^ is true
    const firstElement = isDark ? ".sunIcon" : ".moonIcon";
    const secondElement = isDark ? ".moonIcon" : ".sunIcon";
    animRef.current = tl
      .fromTo(
        firstElement,
        {
  ...
  return (
    <>
      ...
      <Outer onClick={() => clickedOuter()} className="moonIcon">
        <svg

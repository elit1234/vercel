...
const SidebarDark = (): any => {
  const { theme, setTheme } = useTheme();

  useEffect(() => {
    const isDark = theme && theme === "dark" ? true : false;
    const sunIcon: HTMLElement = document.querySelector(".mobileSunIcon")!;
    const moonIcon: HTMLElement = document.querySelector(".mobileMoonIcon")!;
    if (isDark) sunIcon.style.width = "4.5rem";
    else moonIcon.style.width = "4.5rem";
  }, []);
  ...
  const toggleTheme = () => {
    ...
    if (isDark) {
      ...
      moonIcon.style.width = "4.5rem";
    } else {
      moonIcon.style.width = "0";
      sunIcon.style.width = "4.5rem";
    }
  ...
  return (
    <>
      <svg
        className="mobileDarkIcon mobileMoonIcon"
      ...
      <svg
        className="mobileDarkIcon mobileSunIcon"
        xmlns="http://www.w3.org/2000/svg"
        enableBackground="new 0 0 24 24"
        viewBox="0 0 24 24"
        fill="gold"
